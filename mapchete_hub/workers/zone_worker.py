...
def run(
    self, config=None, mode="continue", zoom=None, bounds=None, debug=False,
    multi=cpu_count(), max_chunksize=1
):
    self.send_event('task-progress', current=None, total=None)
    ...
    self.send_event('task-progress', current=0, total=total_tiles)
    logger.debug("total tiles: %s", total_tiles)
    for i, _ in enumerate(executor):
        i += 1
        logger.debug("tile %s/%s finished", i, total_tiles)
        self.send_event('task-progress', current=i, total=total_tiles)
