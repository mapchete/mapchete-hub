...
###################
DATA_DIR="/mnt/data"
# find out name for 350G sized drive
VOLUME="/dev/$(lsblk | grep 350G | head -n1 | sed -e 's/\s.*$//')"
...
retry 10 docker pull registry.gitlab.eox.at/maps/mapchete_hub/mhub-s1:s1_update
...
docker run \
  ...
  -d \
  registry.gitlab.eox.at/maps/mapchete_hub/mhub-s1:s1_update \
